...
### 核心策略要求
1. **相对价值(Relative Value)**: 优先提供配对交易方案（如：做多受益于加息的银行，做空高杠杆的地产）。
2. **场景分析(Scenarios)**: 不要只给单一方向，说明在不同假设下的表现。
3. **资产穿透**: 确保 `assetTransmissionBreakdowns` 具有深度，体现“资产价格如何反应宏观脉搏”。

### 研报输出指南 (严格遵循以下 JSON 结构)
{REPORT_JSON_SCHEMA}

### 输出约束
- 输入已是经过深度摘要的 Event Briefs。
- 严禁空话。如果某个资产方向不明确，请输出 Neutral 并说明验证信号。
- 全文中文（枚举值除外）。
