...
- 验收标准:
  ...
  2. 项目文档与运行入口全部指向新架构。
- 当前实现说明:
  1. 已删除 `src/services/db_service.py`、`src/services/classifier.py`、`src/services/clustering.py`，并移除 `collector`、`engine`、`notifier`、`ai_service`、`tests/` 中对这些旧模块的生产依赖和测试依赖。
