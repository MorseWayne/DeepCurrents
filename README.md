...
- **预测闭环落地**: 支持将 AI 研报中的 `investmentTrends` 自动映射并写入 `predictions`，供后续评分任务回测。
...
*注：`Multi-Agent` 默认从 `asset_symbols.json` 自动挑选一组 symbol 注入实时行情上下文（可通过 `AI_USE_REALTIME_MARKET_CONTEXT` 关闭）；`yfinance` 也用于评分模块。*
