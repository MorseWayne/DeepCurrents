...
- **异步高性能**: 100% Python 3.10+ 重构，基于 `asyncio` 和 `aiohttp` 的非阻塞 I/O 设计，支撑大规模并发采集。
...
DeepCurrents 支持两种运行模式，区别在于 `is_rss_hub` 标记的信息源如何获取数据。

| 模式 | 适用场景 | RSSHub 源行为 |
| :--- | :--- | :--- |
| **直连模式** | 快速测试、本地开发 | 直接访问 `rsshub.app` 公共实例 (可能限速) |
| **自建 RSSHub 模式** | 生产部署、稳定运行 | 通过 `RSSHUB_BASE_URL` 指向自建实例，更快更稳 |

### Docker Compose 一键部署 (推荐)

```bash
# 启动 DeepCurrents + RSSHub + Redis
