...
DeepCurrents 是一个自动化的全球情报聚合与宏观研报生成系统。它从 `src/config/sources.py` 中配置的 **70+ 资讯源（当前 73 个）** 采集碎片化信息，经由威胁分类、事件聚类等分析管线，最终通过 **Multi-Agent (多智能体)** 协作推理引擎合成为结构化的每日宏观策略研报。
...
- **可演进的预测闭环**: `predictions` 表与评分任务已就绪；AI 报告到预测落库的自动链路仍在代码中预留（未默认启用）。
...
- `AIService` 最终汇总阶段当前使用自由文本输出再做 JSON 提取，极端情况下会出现 JSON 解析失败。
- AI 报告中的资产观点尚未自动写入 `predictions`，因此“自动评分闭环”依赖已有预测记录或手动写入。
...
Compose 模式下 `docker-compose.yml` 当前预设 `RSSHUB_BASE_URL=http://127.0.0.1:1200`（与 `network_mode: host` 配套），无需额外配置。
...
*注：`Multi-Agent` 当前使用占位行情上下文，`yfinance` 已用于评分模块。*
