...
- **预测评分闭环**: 自动保存 AI 对资产趋势的研判，并在 24h 后基于真实走势自动回测评分（0-100分），实现 AI 决策质量的持续量化。
...
- **统一日志系统**: 基于 Pino 的模块化日志，支持终端输出与本地文件落盘（`LOG_TO_FILE`）。
...
### 2. 初始化环境 (Node.js & Python)

```bash
# 安装 Node.js 依赖
npm install

# 初始化 Python 虚拟环境并安装 yfinance
python3 -m venv venv
...
| **自动评分** | `0 */4 * * *` | — | 每 4 小时对历史 AI 预测进行真实行情回测 |
...
| `LOG_PRETTY`                   | `true(开发)`  | 终端日志是否使用可读性更高的彩色格式                        |
| `LOG_TO_FILE`                  | `false`     | 是否将日志写入本地文件                                 |
| `LOG_FILE_PATH`                | `logs/deepcurrents.log` | 本地日志文件路径（相对项目根目录）            |
...
- **多智能体协作流 (Macro/Sentiment/Strategist)**
- **yfinance 实时行情集成**
- **AI 预测准确性评分系统 (Scorer)**
- 信息源分级 & 熔断容错
- 威胁分类管线
